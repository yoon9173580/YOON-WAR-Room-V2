...
class NumberOnlyGMEPipeline:
    ...
    def format_number_only(self, amount):
        """숫자만 000으로 끝나게 포맷팅"""
        # 1000 단위로 반내림하여 000으로 끝나게
        rounded = int(amount / 1000) * 1000
        # 콤마 없이 숫자만 반환
    ...
    def generate_number_only_html(self):
        """숫자만 000으로 끝나는 파이프라인 HTML 생성"""
        ...
        <head>
            ...
            <title>YOON MASTER COMMAND — Number Only Pipeline V59</title>
        ...
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <div class="subtitle">Number Only Pipeline V59 — All Numbers End with 000</div>
                </div>
                ...
                <div class="number-notice">
                    <div class="number-title">NUMBER ONLY FORMAT</div>
                    <div class="number-text">
                        모든 숫자가 000으로 끝나는 숫자만 표시됩니다 (예: 213123123123).
                        클릭하면 실제 금액을 볼 수 있습니다.
                    </div>
                </div>
        ...
        for allocation in allocations:
            ...
            html += f"""
                    <div class="scenario-card">
                        <div class="scenario-header">
                            ...
                            <div class="scenario-total">ST 1 Total: <span class="number-only positive" data-amount="{allocation['st1_total']}">{self.format_number_only(allocation['st1_total'])}</span></div>
                        </div>
                        
                        <div class="section-title">ST 2. 즉시 집행금 (조정됨)</div>
                        <div class="allocation-item">
                            <span class="allocation-label">총 집행금:</span>
                            <span class="number-only negative" data-amount="{allocation['st2_total']}">{self.format_number_only(allocation['st2_total'])}</span>
                        </div>
                        <div class="allocation-item">
                            <span class="allocation-label">집행 후 잔액:</span>
                            <span class="number-only positive" data-amount="{allocation['st2_remaining']}">{self.format_number_only(allocation['st2_remaining'])}</span>
                        </div>
                        
                        <div class="section-title">ST 3. 현재가 기준 GME 투입</div>
                        <div class="allocation-item">
                            <span class="allocation-label">CS 현물 ({allocation['cs_shares']:,}주):</span>
                            <span class="number-only negative" data-amount="{allocation['cs_cost']}">{self.format_number_only(allocation['cs_cost'])}</span>
                        </div>
                        <div class="allocation-item">
                            <span class="allocation-label">주당 가격:</span>
                            <span class="number-only" data-amount="{allocation['current_gme_price']}">${allocation['current_gme_price']:.2f}</span>
                        </div>
                        <div class="allocation-item">
                            <span class="allocation-label">Fidelity 옵션:</span>
                            <span class="number-only negative" data-amount="{allocation['fid_option_total']}">{self.format_number_only(allocation['fid_option_total'])}</span>
                        </div>
                        <div class="allocation-item">
                            <span class="allocation-label">Fidelity 총 컨트랙트:</span>
                            <span class="number-only" data-amount="{allocation['fid_total_contracts']}">{allocation['fid_total_contracts']}</span>
                        </div>
            """
                ...
                for strike in allocation['fid_strikes']:
                    html += f"""
                                    <tr>
                                        ...
                                        <td><span class="number-only" data-amount="{strike['premium_per_contract']}">${strike['premium_per_contract']:.2f}</span></td>
                                        <td><span class="number-only negative" data-amount="{strike['cost']}">{self.format_number_only(strike['cost'])}</span></td>
            ...
            html += f"""
                        <div class="allocation-item">
                            ...
                            <span class="number-only negative" data-amount="{allocation['moo_option_total']}">{self.format_number_only(allocation['moo_option_total'])}</span>
                        </div>
                        <div class="allocation-item">
                            <span class="allocation-label">Moomoo 총 컨트랙트:</span>
                            <span class="number-only" data-amount="{allocation['moo_total_contracts']}">{allocation['moo_total_contracts']}</span>
                        </div>
            """
            ...
            html += f"""
                        <div class="allocation-item">
                            ...
                            <span class="number-only" data-amount="{allocation['total_option_contracts']}">{allocation['total_option_contracts']}</span>
                        </div>
                        
                        <div class="allocation-item">
                            <span class="allocation-label">옵션 활용률:</span>
                            <span class="number-only" data-amount="{utilization_pct}">{utilization_pct:.1f}%</span>
                        </div>
                        ...
                        <div class="final-cash-section">
                            ...
                            <div class="final-cash-amount">
                                <span class="number-only positive" data-amount="{allocation['final_cash']}">{self.format_number_only(allocation['final_cash'])}</span>
                            </div>
        ...
        html += f"""
            ...
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function() {{
                    const numberOnlyElements = document.querySelectorAll('.number-only');
                    
                    numberOnlyElements.forEach(element => {{
...
def main():
    pipeline = NumberOnlyGMEPipeline()
    html_content = pipeline.generate_number_only_html()
    
    with open('number_only_pipeline.html', 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("Number Only GME Pipeline generated!")
    print("File saved as: number_only_pipeline.html")
    print("\nNumber Only Format Applied:")
    print("   • All numbers end with 000")
    print("   • No commas, just digits")
    print("   • Click any number to see actual amount")
    print("   • ST 2 adjusted to $600,000")
    print("   • All calculations based on current price")
